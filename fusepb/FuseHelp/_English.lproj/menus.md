...
*Machine > Profiler*||Start or stop the built-in Z80 execution profiler. When profiling is stopped, Fuse prompts for a file and writes one non-zero address per line as `0x1234,456`, where the first value is the address and the second is the cumulative t-state count spent there.
